...
public class JuliaAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
  ...
  private static final int[] INDENT_TO_BLOCK_TOKENS = { JuliaSymbols.TokenELSE,
      JuliaSymbols.TokenELSEIF, JuliaSymbols.TokenEND, JuliaSymbols.TokenRBRACE };
  private static final int[] CONTINUATION_TOKENS = { JuliaSymbols.TokenBACKSLASH,
      JuliaSymbols.TokenCOMMA, JuliaSymbols.TokenSLASH, JuliaSymbols.TokenPLUS,
      JuliaSymbols.TokenMINUS, JuliaSymbols.TokenSTAR };
  private static final int[] REMOVE_IDENTATION_TOKENS = { JuliaSymbols.TokenRDOCBEGIN,
      JuliaSymbols.TokenRDOCEND };
  ...
  public JuliaAutoIndentStrategy(String partitioning, IPreferenceStore store) {
    preferences = new JuliaPreferenceInterpreter(store);
  }

  // private void clearCachedValues() {
  // closeBlocks = preferences.closeBlocks();
  // smartMode = preferences.isSmartMode();
  // }
  ...
  private static int getBlockBalance(IDocument document, int offset) {
    ...
    while (true) {
      ...
      if (begin == JuliaHeuristicScanner.NOT_FOUND && end == JuliaHeuristicScanner.NOT_FOUND)
        return 0;
  ...
  private void closeBlock(IDocument d, DocumentCommand c, String indent, String afterCursor,
  ...
  private String getApropriateBlockEnding(IDocument d, JuliaHeuristicScanner scanner, int offset)
  ...
  public void customizeDocumentCommand(IDocument document, DocumentCommand command) {
    ...
    try {
      ...
      else if (command.text.length() > 1 && preferences.isSmartPaste())
  ...
  private void smartInsertEndOnSpace(IDocument document, DocumentCommand c)
    ...
    if (prevTokenOffset < 0)
      prevTokenOffset = 0;
    String previous = document.get(prevTokenOffset, c.offset - prevTokenOffset).trim();
    ...
    if ((prevTokenOffset + previous.length()) < (c.offset - 1))
      return;
    if (closeBlocks && !isBlockClosed(document, c.offset)) {
  ...
  private String getOption(String qualifier, String key) {
    return new PreferencesLookupDelegate(getProject()).getString(qualifier, key);
  }

  private int getIntOption(String qualifier, String key) {
    return new PreferencesLookupDelegate(getProject()).getInt(qualifier, key);
  }

  private int getVisualTabLengthPreference() {
    return getIntOption(JuliaFormatterPlugin.ID, CodeFormatterConstants.FORMATTER_TAB_SIZE);
  }
  ...
  private void smartIndentOnKeypress(IDocument d, DocumentCommand c) throws BadLocationException {
    ...
    if (Arrays.binarySearch(INDENT_TO_BLOCK_TOKENS, token) >= 0) {
      ...
      if (Arrays.binarySearch(INDENT_TO_BLOCK_TOKENS, previosToken) >= 0
        ...
        String indent = getPreviousLineIndent(d, info.getOffset() - 1, scanner);
  ...
  private String getLineIndent(IDocument d, int offset, JuliaHeuristicScanner scanner) {
    int blockOffset = scanner.findBlockBeginningOffset(offset);
    if (blockOffset != JuliaHeuristicScanner.NOT_FOUND) {
      try {
        return AutoEditUtils.getLineIndent(d, d.getLineOfOffset(blockOffset)) + indentation();
      }
      catch (BadLocationException e) {
        DLTKUIPlugin.log(e);
      }
  ...
  private String indentation() {
    return "  ";
    // return preferences.getIndent();
  }

  private String getBlockIndent(IDocument d, int offset, JuliaHeuristicScanner scanner) {
    int blockOffset = scanner.findBlockBeginningOffset(offset);
    if (blockOffset != JuliaHeuristicScanner.NOT_FOUND) {
      try {
        return AutoEditUtils.getLineIndent(d, d.getLineOfOffset(blockOffset));
      }
      catch (BadLocationException e) {
        DLTKUIPlugin.log(e);
      }
  ...
  private void smartIndentAfterNewLine(IDocument document, DocumentCommand command)
    ...
    int nonWsPos = scanner.findNonWhitespaceForwardInAnyPartition(command.offset, lineEnd);
    ...
    if (previousIsBlockBeginning(document, scanner, command.offset)) {
      ...
      // Auto close blocks
      if (closeBlocks && scanner.isBlockBeginning(line.getOffset(), lineEnd)
          && !isBlockClosed(document, command.offset)) {
        closeBlock(document, command, indent,
            document.get(command.offset, lineEnd - command.offset), scanner);
      }
    }
    else if (previousIsFirstContinuation(document, scanner, command.offset, line.getOffset())) {
  ...
  private boolean hasUnclosedParen(JuliaHeuristicScanner scanner, int offset, int bound) {
    int pos = scanner.findOpeningPeer(offset, bound, '(', ')');
    return pos != JuliaHeuristicScanner.NOT_FOUND;
  }

  private boolean previousIsFirstContinuation(IDocument d, JuliaHeuristicScanner scanner,
      int offset, int bound) throws BadLocationException {
  ...
  private boolean previousIsContinuation(JuliaHeuristicScanner scanner, int offset, int bound) {
    int token = scanner.previousToken(offset, bound);
    return Arrays.binarySearch(CONTINUATION_TOKENS, token) >= 0;
  }

  private boolean previousIsBlockBeginning(IDocument d, JuliaHeuristicScanner scanner, int offset)
      throws BadLocationException {
    ...
    boolean previousIsBlockBeginning = scanner.isBlockBeginning(previousLine.getOffset(),
        previousLineEnd) || scanner.isBlockMiddle(previousLine.getOffset(), previousLineEnd);
    return previousIsBlockBeginning;
  }

  private boolean nextIsIdentToBlockToken(JuliaHeuristicScanner scanner, int offset, int bound) {
  ...
  private void smartPaste(IDocument d, DocumentCommand c) throws BadLocationException {
    ...
    if (lines.length > startFixFrom) {
      ...
      int shift = computeVisualLength(indent) - computeVisualLength(currentIndent);
      StringBuffer result = new StringBuffer();
      for (int i = 0; i < startFixFrom; i++) {
        result.append(lines[i]).append(delimiter);
      }
      for (int i = startFixFrom; i < lines.length - 1; i++) {
        result.append(shiftIdentation(lines[i], shift)).append(delimiter);
      }
  ...
  private String shiftIdentation(String line, int shift) {
    if (shift > 0) {
      return preferences.getIndentByVirtualSize(shift) + line;
    }
    else {
      int pos = 0;
  ...
  private int computeVisualLength(CharSequence indent) {
    final int tabSize = preferences.getTabSize();
    int length = 0;
   ...
   * @throws BadLocationException
   */
  private String getPreviousLineIndent(IDocument d, int offset, JuliaHeuristicScanner scanner)
      throws BadLocationException {
    StringBuffer result = new StringBuffer();
    ...
    int end = scanner.findNonWhitespaceForwardInAnyPartition(start, start + info.getLength());
